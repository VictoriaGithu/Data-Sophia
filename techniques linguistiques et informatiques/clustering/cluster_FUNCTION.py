...
def reveal_clusters(labels, passages_cooccurrence, n_clusters):
    for i in range(n_clusters):
        ...
        print('')
#revoir si on fait une fonction avec des prints ou avec une liste
...
def kmedoids(n, passages_cooccurrence, model):
    vecteur_texte = []
    for passage in passages_cooccurrence:
        vecteur_texte.append(moyenne_vecteurs(passage, model))
    # matrice de données ATTENTION : chaque ligne représente un vecteur)
...
# def tri_reveal(dict_liste): 
#     for i in dict_liste.keys(): 
#         cluster = dict_liste[i]
#         new_list = set()
#         for list_mot in cluster:
#             new_list = new_list.union(set(list_mot))
#     return new_list
